...
make_c_plot <- function(df_c, output_path) {
  ...
  c_plot <- ggplot(data = df_c_wide, aes(
    x = scenario, y = -MLE,
    fill = min_pop
  )) +
    geom_errorbar(
...
make_mortality_plot <- function(df_fut, output_path) {
  ...
  mort_plot <- ggplot(data = df_fut, aes(
    x = year, y = MLE, fill = min_pop,
    shape = scenario,
    alpha = predict
  )) +
    geom_errorbar(
...
prep_df_for_timeseries <- function(df) {
  ...
  df_all_farms <- df %>%
    ...
    dplyr::summarize(
      ...
      sum_inventory = sum(sum_month_inventory,
        nan.rm = TRUE,
        na.rm = TRUE
      ),
...
plot_timeseries <- function(df, output_path) {
  ...
  readr::write_csv(
    df_farms,
    here::here("./data/clean-farm/processed-values-for-scfs-retrospective.csv")
  )
  ...
  salmon <- ggplot(data = df_farms) +
    ...
    scale_fill_manual(
      ...
      labels = c(
        "All Farms", "Humphrey, Sargeaunt, & Doctors",
  ...
  ktf_only_plot <- ggplot(data = only_ktf) +
    geom_line(aes(
      x = year, y = val / 1000000, group = type,
      linetype = type
    )) +
