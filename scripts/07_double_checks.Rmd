...
We want to know which farms are under BATI control. 
```{r, echo=FALSE}
library(tidyverse)
library(here)
library(patchwork)
...
```
The above farms printed here are all the farms that are in the `.csv` file that Sean provided to Cole from the Nation. We can get GPS points for those farms from the open government data on average numbers of lice on farms:
...
```
So the above farms are those that are in the DFO data from 2011 onwards that are *NOT* in the BATI dataset. 

In addition there is data from Marty (2010):
```{r}
...
marty_cov_plot
```
The farms that are in the DFO data that BATI doesn't have data for are:

```{r}
print(temp_non_BATI_farms)
```
