...
collated_df <- readr::read_csv(here::here(cfg$path$lice_counts))
farm_df <- readr::read_csv(here::here(cfg$path$farm_data))
...
simple_mod <- stats::lm(
  log10(mean_all_leps) ~ log10(marty_mean_lep_tot),
...
wild_farm_ktc_reg <- ggplot(data = yearly_ktc_lice_data) +
  ...
  geom_smooth(
    aes(x = mean_farm_lice, y = mean_wild_lice),
    formula = y ~ x,
    method = "lm"
  ) +
