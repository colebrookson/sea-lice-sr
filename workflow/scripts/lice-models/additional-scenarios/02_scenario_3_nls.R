#' DESCRIPTION: Bateman-style reproduction. All chalimus and all unidentified 
...
nls_fit <- stats::nls(
    y_lep ~ 1 - exp(-c * x_mot), data = fit_df, start = list(c = c0)
)
...
readr::write_csv(
    props_s3,
    here::here("./data/scfs-data/clean/lep-proportions-scenario3.csv")
)
...
replicates <- purrr::map_dfr(
    seq_len(M),
    \(m) impute_once(fish_df, props_s3, seed = 20260120 + m, by = "year") %>%
        dplyr::mutate(rep = m)
)
readr::write_csv(
    replicates,
    paste0(here::here("./data/scfs-data/clean/"), 
    "lice-counts-imputed-replicates-scenario3.csv")
)
...
if (nrow(bad) > 0) stop(nrow(bad), " fish fail per-stage louse conservation.")
...
readr::write_csv(
    r1 %>%
        ...
        dplyr::select(-rep),
    paste0(here::here("./data/scfs-data/clean/"), 
    "lice-counts-for-regression-scenario3.csv")
)
...
p_s3 <- ggplot() +
    ...
    geom_point(
        data = pred_pt, aes(x_mot, y_lep),
        shape = 21, colour = "black", fill = "purple", size = 4
    ) +
...
save_fig(
    ...
    width = 8, height = 6,
    caption = paste(
        "Fig. S3 (Scenario 3). Asymptotic nls fit Y = 1 - exp(-c*X), a = 1 and",
