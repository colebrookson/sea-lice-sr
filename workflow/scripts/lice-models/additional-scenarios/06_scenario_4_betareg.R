...
p_mot_2001_point <- as.numeric(
  stats::predict(
    beta_fit,
    newdata = tibble::tibble(x_mot = x_2001),
    type = "response"
  )
)
...
readr::write_csv(props_point, here::here(cfg$path$scenarios$scen_4_props_point))
readr::write_csv(props_lower, here::here(cfg$path$scenarios$scen_4_props_lower))

# impute both variants ---------------------------------------------------------
M <- cfg$run$num_M

run_variant <- function(props, replicates_path, counts_path) {
  reps <- purrr::map_dfr(
    seq_len(M),
    \(m) {
      impute_once(fish_df, props, seed = 20260120 + m, by = "year") |>
        dplyr::mutate(rep = m)
    }
  )
  readr::write_csv(reps, replicates_path)
...
run_variant(
  props_point,
  here::here(sprintf(cfg$path$scenarios$scen_4_reps, "point")),
  here::here(sprintf(cfg$path$scenarios$scen_4, "point"))
)
run_variant(
  props_lower,
  here::here(sprintf(cfg$path$scenarios$scen_4_reps, "lower")),
  here::here(sprintf(cfg$path$scenarios$scen_4, "lower"))
)
...
p_s4 <- ggplot() +
  geom_ribbon(
    data = grid,
    aes(x_mot, ymin = lower, ymax = upper),
    fill = "grey80",
    alpha = 0.7
  ) +
  ...
  geom_point(
    data = pred_pts,
    aes(x_mot, y_lep, fill = kind),
    shape = 21,
    colour = "black",
    size = 4
  ) +
  scale_fill_manual(values = c("point" = "purple", "lower 95%" = "steelblue")) +
...
save_fig(
  ...
  height = 6,
  caption = paste(
    "Scenario 4. Beta regression (logit link) relating the yearly",
    ...
    "fit years (2002-2025); the purple point is the predicted 2001 value",
    "and the blue point its lower 95% bound, which seeds the sensitivity",
    "run.",
    sprintf(
      "2001 proportion = %.3f (point), %.3f (lower).",
      p_mot_2001_point,
      p_mot_2001_lower
    )
  )
