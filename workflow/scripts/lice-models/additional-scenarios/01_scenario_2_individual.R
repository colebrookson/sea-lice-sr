...
mot_model <- stats::glm(
    ...
    family = stats::binomial(link = "logit"), data = mot_fit_df
)
cope_model <- stats::glm(
    cbind(lep_cope_obs, cal_cope_obs) ~ all_lice,
    family = stats::binomial(link = "logit"), data = cope_fit_df
)
...
emp_props <- fish_df %>%
    ...
    dplyr::summarize(
        p_mot_emp = dplyr::if_else(
            ...
            empirical_prop(lep_mot_obs, sp_mot, POOLED), NA_real_
        ),
        p_cope_emp = dplyr::if_else(
            dplyr::first(cope_speciated),
            empirical_prop(lep_cope_obs, sp_cope, POOLED), NA_real_
        ),
        .groups = "drop"
    )
...
props_s2$p_mot_fish  <- stats::predict(
    mot_model,  newdata = props_s2, type = "response"
)
props_s2$p_cope_fish <- stats::predict(
    cope_model, newdata = props_s2, type = "response"
)
...
replicates <- purrr::map_dfr(
    seq_len(M),
    \(m) impute_once(fish_df, props_s2, seed = 20260120 + m, by = "obs_id") %>%
        dplyr::mutate(rep = m)
)
readr::write_csv(
    replicates,
    paste0(here::here("./data/scfs-data/clean/"), 
    "lice-counts-imputed-replicates-scenario2.csv")
)
...
readr::write_csv(
    r1 %>%
        ...
        dplyr::select(-rep),
    paste0(here::here("./data/scfs-data/clean/"), 
    "lice-counts-for-regression-scenario2.csv")
)
