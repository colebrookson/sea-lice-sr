...
raw <- readr::read_csv(
    ...
    standardize_names(.) %>% 
    dplyr::select(year, day, month, location, dplyr::all_of(
...
spec_2001 <- fish_df %>%
    ...
    dplyr::summarise(dplyr::across(
        dplyr::all_of(c("lep_pamale", "lep_pafemale", "lep_male",
                        "lep_nongravid", "lep_gravid", "cal_mot", 
                        "cal_gravid")),
        \(x) sum(x, na.rm = TRUE)
    ))
if (any(spec_2001 > 0)) {
...
fish_df <- fish_df %>% 
    dplyr::mutate(
        ...
        all_lice = rowSums(
            cbind(all_cope, all_mot, all_chal), na.rm = TRUE
        )
    )
