...
yr_shared <- glmmTMB::fixef(tmb_fit)$cond[grep(
  "^year_f",
  names(glmmTMB::fixef(tmb_fit)$cond)
)]
yr_unstr <- glmmTMB::fixef(fit_slope)$cond[grep(
  "^year_f",
  names(glmmTMB::fixef(fit_slope)$cond)
)]
yr_diag <- glmmTMB::fixef(fit_diag)$cond[grep(
  "^year_f",
...
se_shared <- se(tmb_fit)
se_unstr <- se(fit_slope)
se_diag <- se(fit_diag)
...
cat(
  "SE ratio unstr/shared — range:",
  round(range(se_unstr / se_shared), 3),
  "\n"
)
cat("SE ratio diag/shared — range:", round(range(se_diag / se_shared), 3), "\n")
...
# converged?
lapply(list(shared = f_wk_shared, diag = f_wk_diag, unstr = f_wk_unstr), \(m) {
  m$sdr$pdHess
})
...
yr_s <- glmmTMB::fixef(f_wk_shared)$cond[grep(
  "^year_f",
  names(glmmTMB::fixef(f_wk_shared)$cond)
)]
yr_u <- glmmTMB::fixef(f_wk_unstr)$cond[grep(
  "^year_f",
  names(glmmTMB::fixef(f_wk_unstr)$cond)
)]
yr_d <- glmmTMB::fixef(f_wk_diag)$cond[grep(
  "^year_f",
  names(glmmTMB::fixef(f_wk_diag)$cond)
)]
...
cat(
  "SE ratio wk unstr/shared — range:",
  round(range(se(f_wk_unstr) / se(f_wk_shared)), 3),
  "\n"
)
cat(
  "SE ratio wk diag/shared — range:",
...
predict_one <- function(fit, label, stages, years) {
  ...
  pr <- predict(fit, newdata = grid, se.fit = TRUE, re.form = NA, type = "link") # population-level, link scale
...
sapply(list(orig = tmb_fit, best = f_wk_diag), \(f) {
  re_var_by_stage(f, stage_levels)
})
...
pred_two$panel <- factor(
  ifelse(
    grepl("shared  wk:shared", pred_two$model),
    "Original (both shared)",
    "Best (both stage-specific)"
  ),
...
save_fig(
  ...
  caption = paste(
    ...
    "note the upward shift and widened intervals in the best model."
  )
)

# does year x stage matter?
#' beta_year assumes year effects are common across stages, shifted only by a
#' stage constant. 2001 cope is empty, so build the
#' interaction as a factor and droplevels rather than 0 + year_f:stage.
collated_df_long <- collated_df_long %>%
  dplyr::mutate(ys_f = droplevels(interaction(year_f, stage, drop = TRUE)))

f_ys_add <- glmmTMB::glmmTMB(
  count ~ 0 + year_f + stage + (0 + stage || week_f) + (0 + stage || ly_f),
  family = glmmTMB::nbinom2,
  data = collated_df_long
)
f_ys_int <- glmmTMB::glmmTMB(
  count ~ 0 + ys_f + (0 + stage || week_f) + (0 + stage || ly_f),
  family = glmmTMB::nbinom2,
  data = collated_df_long
)

AIC(f_ys_add, f_ys_int)
anova(f_ys_add, f_ys_int)

mot_int <- glmmTMB::fixef(f_ys_int)$cond[grep(
  "\\.mot$",
  names(glmmTMB::fixef(f_ys_int)$cond)
)]
mot_add <- glmmTMB::fixef(f_ys_add)$cond[grep(
  "^year_f",
  names(glmmTMB::fixef(f_ys_add)$cond)
)]

se_of <- function(m, pat) {
  v <- sqrt(diag(vcov(m)$cond))
  v[grep(pat, names(glmmTMB::fixef(m)$cond))]
}

ys_compare <- data.frame(
  est_add = round(mot_add, 3),
  est_int = round(as.numeric(mot_int), 3),
